...
internal class sky72 : club
{
	private TesseractEngine engine;

	private DateTime opentime;

	private const string loginUrl = "https://onetheclub.com/member/login";

	private Random rand = new Random();

	public sky72(int pManagerId)
	{
	...
	public override bool login(Form1 _frm)
	{
		...
		try
		{
			...
			((WebDriver)drv).Navigate().GoToUrl("https://onetheclub.com/member/login");
			Thread.Sleep(500);
			...
			lock (WebDriverExtensions.lockObject)
			{
				...
				Thread.Sleep(200);
				new Actions((IWebDriver)(object)drv).SendKeys(Keys.Enter).Perform();
				try
				{
	...
	public override OpResult bookOne()
	{
		...
		((WebDriver)drv).Navigate().GoToUrl("https://onetheclub.com/reservation/golf");
		((IWebDriver)(object)drv).FindElement(By.Id("companyD01")).Click();
		while (num >= 0 && !frm.stopClicked)
		{
			try
			{
				IWebElement val2 = ((IWebDriver)(object)drv).FindElement(By.Id(text), 10);
				...
				val2.Click();
				Thread.Sleep(300);
				ReadOnlyCollection<IWebElement> readOnlyCollection = ((IWebDriver)(object)drv).FindElements(By.XPath(".//button[contains(@id, 'J5720')]"), 20);
