...
internal class century : club
{
	private TesseractEngine engine;

	private const string loginUrl = "http://member.century21cc.co.kr/html/member/login_member.asp";

	public century(int pManagerId)
	{
	...
	public override bool login(Form1 _frm)
	{
		...
		try
		{
			...
			((WebDriver)drv).Navigate().GoToUrl("http://member.century21cc.co.kr/html/member/login_member.asp");
			Thread.Sleep(500);
			lock (WebDriverExtensions.lockObject)
			{
				((IWebDriver)(object)drv).FindElement(By.XPath("//*[@id='loginId']")).SendKeys(id);
				((IWebDriver)(object)drv).FindElement(By.XPath("//*[@id='loginPw']")).SendKeys(pwd);
				WebDriverExtensions.clickLock(((IWebDriver)(object)drv).FindElement(By.XPath("//*[@id='loginForm']/fieldset/button")));
				Thread.Sleep(200);
	...
	private string selectDate(string date)
	{
		...
		return $"//*[@id='main']/section[2]/div/div/div[1]/div[2]/table/tbody/tr[{row}]/td[{col}]/a";
	}
	...
	public override OpResult bookOne()
	{
		...
		try
		{
			WebDriverExtensions.clickLock(((IWebDriver)(object)drv).FindElement(By.ClassName("nextmonth")));
			Thread.Sleep(500);
